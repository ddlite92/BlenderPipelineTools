...
# Ensure Cryptomatte output paths are correctly set for each frame
def ensure_cryptomatte_output(scene_directory, file_name_without_extension):
    cryptomatte_directory = scene_directory / "Cryptomatte_"
    cryptomatte_directory.mkdir(parents=True, exist_ok=True)

    # Iterate through each render layer and set the Cryptomatte output path
    for scene in bpy.data.scenes:
        for layer in scene.view_layers:
            if layer.use_pass_cryptomatte_object:
                # Update Cryptomatte EXR file path for each frame of the animation
                cryptomatte_path = str(cryptomatte_directory / (file_name_without_extension + "_Cryptomatte_"))
                scene.render.filepath = cryptomatte_path
                print(f"Cryptomatte render path set for layer '{layer.name}' to: {scene.render.filepath}")

                # Force the file path on each frame render
                for node in scene.node_tree.nodes:
                    if node.type == 'OUTPUT_FILE' and 'Cryptomatte_' in node.label:
                        node.base_path = cryptomatte_path
                        print(f"Node '{node.name}' Cryptomatte output set to: {node.base_path}")
...
class OT_RunOutputRenamer(Operator):
    bl_idname = "wm.run_output_renamer"
    bl_label = "Run Output Renamer"
    
    def execute(self, context):
        base_output_directory = Path(context.scene.output_renamer_props.base_output_directory)
        main(base_output_directory)
        return {'FINISHED'}
...
def register():
    ...
    bpy.utils.register_class(OT_RunOutputRenamer)
...
def unregister():
    ...
    bpy.utils.unregister_class(OT_RunOutputRenamer)
